...
update self = case _ of
  ...
  CloneSnapshot -> guard (isNothing self.state.error) $ launchAff_ do
    let fsName = self.state.base <> "/" <> self.state.fsName
...
cloneSnapshot = make component { initialState, didMount, render }
  ...
  where
    ...
    didMount self =
      self.setState _ { base = S.takeWhile ((/=) $ S.codePointFromChar '/') self.props.dataset.name }
