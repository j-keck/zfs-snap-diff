...
update self = case _ of
  Diff -> launchAff_ $ do
    ...
    liftEffect $ self.setState _ { diff = res }
...
viewDiff = make component { initialState, render, didMount }
  where
    ...
    initialState = { view: InlineDiff, diff: Left (Bug "Uninitialized") }
    ...
    inlineDiff self =
      case self.state.diff of
        Right diff -> fragment $ flip map (zipWithIndex diff.inlineDiffHTMLFragment) \(Tuple idx html) ->
                      R.div
                      { className: "m-1"
                      , children:
                        [ actionButton
...
zipWithIndex as = zipWith Tuple (range 0 (length as - 1)) as
