...
update self = case _ of
  ...
  CreateSnapshot -> guard (S.null self.state.error) $
    launchAff_ $ Dataset.createSnapshot self.props.dataset self.state.snapshotName
...
createSnapshotModal = make component { initialState, didMount, render }
  ...
  where
    ...
    didMount self = self.setState _ { snapshotTemplate = self.props.defaultTemplate }
...
validateName name =
  ...
  let validStrs = (toEnumWithDefaults bottom top >>> S.singleton <$> 48..57 <> 65..90<>97..122)
                  <> ["_", "-", ":", "."]
      invalidStrs = A.filter (flip A.elem validStrs >>> not) $ (S.singleton <$> S.toCodePointArray name)
  in if A.null invalidStrs
     then Right name
     else Left $ "Invalid character found: " <> (S.joinWith ", " invalidStrs)
...
parse s
  | S.null s          = []
  | S.take 1 s == "%" = A.cons (either Invalid Fmt $ decode $ S.take 2 s)
                               (parse $ S.drop 2 s)
  | otherwise         = let plain = S.takeWhile (_ /= S.codePointFromChar '%') s
                        in A.cons (Plain plain) (parse $ S.drop (S.length plain) s)
